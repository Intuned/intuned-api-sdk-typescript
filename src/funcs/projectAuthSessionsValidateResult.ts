...
export function projectAuthSessionsValidateResult(
  ...
  Result<
    operations.ValidateAuthSessionResultResponseBody,
    | IntunedClientError
...
> {
  const input: operations.ValidateAuthSessionResultRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.ValidateAuthSessionResultRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ValidateAuthSessionResultResponseBody$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
