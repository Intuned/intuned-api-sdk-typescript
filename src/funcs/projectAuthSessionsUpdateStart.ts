 ...
 */
export async function projectAuthSessionsUpdateStart(
  ...
  Result<
    components.CreateOrUpdateAuthSessionStart,
  ...
  >
> {
  const input$: operations.UpdateAuthSessionStartRequest = {
  ...
  const parsed$ = schemas$.safeParse(
    input$,
    (value$) =>
      operations.UpdateAuthSessionStartRequest$outboundSchema.parse(value$),
  ...
  const body$ = encodeJSON$("body", payload$.UpdateAuthSessionRequest, {
    explode: true,
  });
  ...
  const path$ = pathToFunc(
    "/{workspaceId}/projects/{projectName}/auth-sessions/update",
  )(pathParams$);
  ...
  const [result$] = await m$.match<
    ...
    | ConnectionError
  >(
    m$.json(201, components.CreateOrUpdateAuthSessionStart$inboundSchema),
