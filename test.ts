...
import { AuthSessionCreateDoneResult, AuthSessionCreateFailedResult, AuthSessionCreateResult, FileSource, FileT } from './src/models/components';
...
async function testJobs(client: IntunedClient, projectName: string) {
    ...
    const triggerJob2 = await jobs.trigger(projectName, jobId).catch((error) => error);
    console.log("Trigger job 2", triggerJob2);
    const resumeJob = await jobs.resume(projectName, jobId);
    console.log("Resume job", resumeJob);
    const triggerJob3 = await jobs.trigger(projectName, jobId).catch((error) => error);
    ...
    const terminateRun = await jobs.runs.terminate(projectName, jobId, lastRun!.id);
...
async function testQueues(client: IntunedClient, projectName: string) {
    ...
    async function appendAndGetResult(apiName: string) {
        ...
        while(true) {
            ...
            const getResult = await queues.items.result(projectName, queueId, appendItem.runId);
    ...
    const [appendSuccess, appendFail] = await Promise.all([appendAndGetResult('api'), appendAndGetResult('api2')]);
    await queues.items.delete(projectName, queueId, appendSuccess).catch((error) => console.log("Delete success item", error));
    await queues.items.delete(projectName, queueId, appendFail).catch((error) => console.log("Delete fail item", error));

    const repeatItemAppend = await queues.repeatItems.append(projectName, queueId, {
        apiName: "api",
        parameters: {},
        repeat: "10 minutes",
    });
    ...
    const repeatItemUpdate = await queues.repeatItems.update(projectName, queueId, repeatItemAppend.id, {
        apiName: "api",
        parameters: {},
        repeat: "20 minutes",
    });
    console.log("Repeat item update", repeatItemUpdate);
    const repeatItemDelete = await queues.repeatItems.delete(projectName, queueId, repeatItemAppend.id);
...
async function testAuthSessions(client: IntunedClient, projectName: string) {
    ...
    while(true) {
        ...
        if(previousResult === undefined || previousResult.status === "pending") {
            const getCreate = await authSessions.create.result(projectName, startCreate.operationId!);
            console.log("Get create auth session", getCreate);
            previousResult = getCreate;
            continue;
        }
        if(previousResult.status === "requested_more_info") {
            const resumeCreate = await authSessions.create.resume(projectName, startCreate.operationId!, {
