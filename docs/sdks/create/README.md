...
async function run() {
  const result = await intunedClient.project.authSessions.create.result("my-project", "ffff91cb-e4ce-425d-8dca-c3b005b8acad");
...
async function run() {
  const result = await intunedClient.project.authSessions.create.resume("my-project", "5058bd8e-c7dd-4be7-8da7-a4222efac647", {
    input: "123456",
